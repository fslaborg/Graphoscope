...
type ArrayAdjacencyGraph<'Vertex,'Label,'Edge when 'Vertex : equality and 'Edge : equality> internal (vertexEdges:Dictionary<'Vertex,ResizeArray<LEdge<'Vertex,'Edge>>>, labels:Dictionary<'Vertex,'Label>) =
    ...
    let vertexOutEdges :Dictionary<'Vertex,ResizeArray<LEdge<'Vertex,'Edge>>> = vertexEdges

    new () = ArrayAdjacencyGraph(new Dictionary<'Vertex,ResizeArray<LEdge<'Vertex,'Edge>>>(), new Dictionary<'Vertex,'Label>())
    ...
    new(vertices : LVertex<'Vertex,'Label> list,edges : LEdge<'Vertex,'Edge> list) =
        ...
        let vertexEdges = System.Collections.Generic.Dictionary<'Vertex,ResizeArray<LEdge<'Vertex,'Edge>>>()
        ...
        for i=0 to vertexArray.Length-1 do
            ...
            let edges           = 
                let edegResizeArray :LEdge<'Vertex,'Edge>ResizeArray = ResizeArray<LEdge<'Vertex,'Edge>>()
                edgeArray
                |> Array.map (fun (s, t, w) -> 
                    if s=vertex || t=vertex then
                        edegResizeArray.Add(s,t,w)
                    else
                        ()
                ) 
                |>ignore
    ...
    member this.Copy() :ArrayAdjacencyGraph<'Vertex,'Label,'Edge> =
        ...
        let newEdges = 
            vertexOutEdges 
            |> Dictionary.copyRecursive (fun x-> 
                let newArray :LEdge<'Vertex,'Edge>ResizeArray = ResizeArray<LEdge<'Vertex,'Edge>>()
                for i in x do
                    newArray.Add(i)
                newArray
            ) //Array.copy
    ...
    member this.GetEdge((source:'Vertex),(target:'Vertex)) :LEdge<'Vertex,'Edge> =
        try 
            (vertexOutEdges.Item source).Find (fun (s,t,e) -> s = source && t = target)
        with
    ...
    member this.GetUndirectedEdge((source:'Vertex),(target:'Vertex)) :LEdge<'Vertex,'Edge> =
        try 
            (vertexOutEdges.Item source).Find (fun (s,t,e) -> s = source && t = target || t = source && s = target)
        with
        | _ -> failwithf "An edge from vertex %O to vertex %O does not exist in the graph" source target
    ...
    member this.GetEdges((source:'Vertex),(target:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray =
        try 
            (vertexOutEdges.Item source).FindAll (fun (s,t,e) -> s = source && t = target)
        with
        |_ -> failwithf "An edge from vertex %O to vertex %O does not exist in the graph" source target

    ///Return all edges in the graph that matches the conditions. Undirected adaptation of GetEdges.
    member this.GetUndirectedEdges((source:'Vertex),(target:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray =
        try 
            (vertexOutEdges.Item source).FindAll (fun (s,t,e) -> s = source && t = target|| t = source && s = target)
        with
        |_ -> failwithf "An edge from vertex %O to vertex %O does not exist in the graph" source target
    ...
    member this.ContainsEdge(edge:LEdge<'Vertex,'Edge>) :bool =            
        ...
        try 
            let array = (vertexOutEdges.Item source) //.ToArray()
            (array.Contains (edge)) || (array.Contains (target,source,weight))
        with 
        |_ -> false


    ///Lookup all edges connected to the vertex v in the graph, returning a Some value if a binding exists and None if not.
    member this.TryGetConnectedEdges((v:'Vertex)) :ResizeArray<LEdge<'Vertex,'Edge>> option =
    ...
    member this.GetConnectedEdges((v:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray =
        try
    ...
    member this.TryGetInEdges((v:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray option=      
        Dictionary.tryGetValue v vertexOutEdges
        |> Option.map (fun x -> 
            x.FindAll (fun (s, t, w) -> t=v)
        )

    ///Lookup all edges that target the vertex v in the graph, returning an array of connected edges.
    member this.GetInEdges((v:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray =
        try
            (vertexOutEdges.Item v).FindAll (fun (s, t, w) -> t=v)
        with
        |_ -> failwithf "The vertex %O does not exist in the graph." v
    ...
    member this.TryGetOutEdges((v:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray option=
        Dictionary.tryGetValue v vertexOutEdges    
        |> Option.map(fun x -> 
            x.FindAll (fun (s, t, w) -> s=v)
        )

    ///Lookup all edges that originate from the vertex v in the graph, returning an array of connected edges.
    member this.GetOutEdges((v:'Vertex)) :LEdge<'Vertex,'Edge> ResizeArray =
        try
            (vertexOutEdges.Item v).FindAll(fun (s, t, w) -> s=v)
        with
        |_ -> failwithf "The vertex %O does not exist in the graph." v
    ...
    member this.SetWeight((source:'Vertex),(target:'Vertex),(weight:'Edge)) =
        let sourceIndex = 
            try
                ...
                |> fun x -> x.FindIndex (fun (s, t, w) -> t=target)
            with
            |_ -> failwithf "An edge from vertex %O to vertex %O does not exist in the graph" source target
        ...
        let targetIndex = 
            try
                ...
                |> fun x -> x.FindIndex (fun (s, t, w) -> s=source)
            with
            |_ -> failwithf "An edge from vertex %O to vertex %O does not exist in the graph" source target
    ...
    member this.AddEdge((s, t, w): LEdge<'Vertex,'Edge>) =
        ...
        edgeArraySource.Add (s,t,w)
        edgeArrayTarget.Add (s,t,w)
    ...
    member this.RemoveEdge((s, t, w): LEdge<'Vertex,'Edge>) =
        ...
        //         |_ -> failwithf "The target vertex %O of the edge does not exist in this graph." t        
            ...
            (vertexOutEdges.Item s).Remove (s,t,w)|>ignore
            (vertexOutEdges.Item t).Remove (s,t,w)|>ignore
    ...
    member this.RemoveManyEdges (edges:LEdge<'Vertex,'Edge>ResizeArray) =
    ...
    member this.AddVertex ((v, l): LVertex<'Vertex,'Label>)=
        vertexOutEdges.Add (v,ResizeArray<LEdge<'Vertex,'Edge>>())  |> ignore
    ...
    member this.Predecessors(v:'Vertex) :'Vertex[] =
        try
            (this.GetConnectedEdges v).ToArray()
            |> Array.choose (fun (s,t,w) -> if t=v then Some s else None)
            |> Array.distinct
        with
        |_ -> failwithf "The vertex %O does not exist in the graph." v
...
module Edges =
    ...
    ///Return all edges in the graph that matches the conditions
    let getMany (source:'Vertex) (target:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray =
    ...
    ///Return all edges in the graph that matches the conditions
    let getManyUndirected (source:'Vertex) (target:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray =
    ...
    ///Lookup all edges connected to the vertex v in the graph, returning a Some value if a binding exists and None if not.
    let tryGetConnected (v:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray option =
       graph.TryGetConnectedEdges v
       
    ///Lookup all edges connected to the vertex v in the graph, returning an array of connected edges.
    let getConnected (v:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray =
        graph.GetConnectedEdges v

    ///Lookup all edges that target the vertex v in the graph, returning a Some value if a binding exists and None if not.
    let tryGetInEdges (v:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray option=      
        graph.TryGetInEdges v

    ///Lookup all edges that target the vertex v in the graph, returning an array of connected edges.
    let getInEdges (v:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray =
        graph.GetInEdges v

    ///Lookup all edges that originate from the vertex v in the graph, returning a Some value if a binding exists and None if not.
    let tryGetOutEdges (v:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray option=
        graph.TryGetOutEdges v

    ///Lookup all edges that originate from the vertex v in the graph, returning an array of connected edges.
    let getOutEdges (v:'Vertex) (graph: ArrayAdjacencyGraph<'Vertex,'Label,'Edge>) :LEdge<'Vertex,'Edge> ResizeArray =
        graph.GetOutEdges v
